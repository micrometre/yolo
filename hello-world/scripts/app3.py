...
class Config:
    ...
    video_path: str = "../videos/alprVideo2.mp4"  # Add video path
...
class PlateDetector:
    ...
    def _process_plates(self, img: np.ndarray, plates: List, image_name: str) -> List[Dict[str, Any]]:
        ...
        for (x, y, w, h) in plates:
            ...
            # Process with EasyOCR instead of pytesseract
            ...
            plates_data.append({
                "plate_text": text,
                "confidence": float(results[0][2]) if results else 0.0,  # Add confidence score
                "coordinates": {
                    "x": int(x),
                    "y": int(y),
                    "width": int(w),
