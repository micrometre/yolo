...
class Config:
    ...
    video_path: str = "../videos/alprVideo.mp4"  # Add video path
    video_extensions: tuple = ('.mp4', '.avi', '.mov')  # Add supported video formats
    ...
    model_path: str = "../../weights/yolov4-tiny.weights"
...
class PlateDetector:
    ...
    def _process_plates(self, img: np.ndarray, plates: List, image_name: str) -> List[Dict[str, Any]]:
        ...
        for (x, y, w, h) in plates:
            ...
            # Draw the plate text above the rectangle
            if text:
                # Calculate position for text (above the rectangle)
                text_x = x
                text_y = y - 15  # 15 pixels above the rectangle
            ...
            plates_data.append({
                "plate_text": text,
                "confidence": float(results[0][2]) if results else 0.0,
                "coordinates": {
                    "x": int(x),
                    "y": int(y),
                    "width": int(w),
    ...
    def process_video(self, video_path: str) -> Dict[str, Any]:
        ...
        output_path = os.path.join(self.images_dir, f"processed_{os.path.basename(video_path)}")
        ...
        results = {
            ...
            "processed_video": os.path.relpath(output_path, self.config.output_dir),
        ...
        while cap.isOpened():
            ...
            if frame_count % self.config.frame_skip == 0:
                ...
                self._draw_labels(boxes, confidences, class_ids, frame)

                # Detect and process plates
                plates = self._detect_plates(frame)
                plates_data = self._process_plates(
                    frame, 
                    plates, 
                    f"frame_{frame_count:06d}"
                )

                results["plates"].extend(plates_data)
                results["total_plates"] += len(plates_data)
...
def main():
    ...
    if os.path.exists(config.video_path):
        ...
        results = detector.process_video(config.video_path)
        
        # Save results
        if results:
            json_file = os.path.join(config.output_dir, "video_detection_results.json")
            with open(json_file, "w") as f:
                json.dump(results, f, indent=4, cls=NumpyEncoder)
            print(f"Processing complete! Results saved to {json_file}")
    else:
